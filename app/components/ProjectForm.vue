<template>
  <form class="max-w-3xl mx-auto space-y-10" @submit.prevent="onSubmit">
    <!-- BASIC INFO -->
      ...
      <div class="flex gap-3">
        <div class="flex-1">
          <BaseInput
            ...
            @keydown.enter.prevent="handleAddExperience"
          />
        </div>
        <BaseBtn
          type="button"
    ...
    <section class="space-y-6">
      ...
      <div>
        <label class="block text-sm font-bold text-secondary-500 mb-2"
          ...
          <span
            v-if="mode === 'create'"
            class="text-secondary-700 font-normal ml-1 normal-case"
            >(required, displayed on project card)</span
          >
          <span v-else class="text-secondary-700 font-normal ml-1 normal-case"
            >(remove to replace - a new image is required)</span
          >
        </label>
        ...
        <FileUpload
          ...
          :disabled="isSubmitting"
          with-alt-text
          @update:file-list="onMainImageFileListUpdate"
      ...
      <div>
        ...
        <FileUpload
          ...
          :disabled="isSubmitting"
          with-alt-text
          @update:file-list="onOtherImagesFileListUpdate"
...
import type { ProjectFormProps } from "~/types/components";
...
const handleAddTechnology = () => {
  props.addTechnology(techInput.value);
  techInput.value = "";
};

const handleAddExperience = () => {
  props.addExperience(experienceInput.value);
